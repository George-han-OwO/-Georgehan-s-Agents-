...
const seedTasks: Task[] = [
  { id: 'T-028', title: '研究多 Agent 协作产品', assignee: 'Mochi', status: '执行中', progress: 46, eta: '12 分钟', priority: '高', path: ['Mochi', 'Nova', 'Lumi'], reason: '产品策划能力匹配，当前负载最低' },
  { id: 'T-027', title: '为首页准备首屏文案', assignee: 'Muse', status: '执行中', progress: 72, eta: '6 分钟', priority: '中', path: ['Muse'], reason: '视觉与品牌能力匹配' },
  { id: 'T-026', title: '批准生产环境读取权限', assignee: '用户', status: '等待用户', progress: 34, eta: '等待决定', priority: '高', path: ['Pixel', 'Mochi'], reason: '涉及外部系统权限，需要用户确认' },
  { id: 'T-025', title: '整理昨日运行报告', assignee: 'Quill', status: '已完成', progress: 100, eta: '已交付', priority: '低', path: ['Quill'], reason: '写作与总结能力匹配' },
  { id: 'T-024', title: '检查三台设备心跳', assignee: 'Dozer', status: '待认领', progress: 0, eta: '约 3 分钟', priority: '中', path: [], reason: '等待值守 Agent 唤醒' },
];
...
const statusOrder: Task['status'][] = ['待认领', '执行中', '等待用户', '已完成'];
...
export default function Home() {
  ...
  const send = () => {
    ...
    if (taskMode) {
      ...
      const task: Task = {
        ...
        title: text.replace(/@\S+/g, '').trim(), assignee: agent.name, status: '执行中', progress: 8,
  ...
  const resolveCheckpoint = (outcome: 'partial' | 'ask') => {
    ...
    flash(outcome === 'partial' ? '已提交带局限说明的阶段结果' : '已向用户生成一次合并询问');
  };
  ...
  return (
    <main className={`war-room ${paused ? 'is-paused' : ''}`}>
      <header className="topbar">
        ...
        <div className="room-title"><span className={`live-dot ${apiSummary.status !== 'online' ? 'muted' : ''}`} /><div><strong>{USER_NAME} 的 Agent 们</strong><small>{paused ? '全局已暂停' : apiSummary.status === 'online' ? `接口在线 · ${apiSummary.devices} 台真实设备已接入` : '正在连接通用接口…'}</small></div></div>
        <div className="header-actions">
          ...
          <button className="user-chip"><span>用</span> {USER_NAME}</button>
        </div>
      </header>
        ...
        <div className="side-section">
          ...
          {allTasks.filter((task) => task.status === '执行中').slice(0, 3).map((task, index) => (
            <button key={task.id} className={`mini-task ${index === 0 ? 'active' : ''}`} onClick={() => { setSelectedTask(task); setSelectedAgent(null); }}>
              <span className={`task-glyph ${index ? 'coral' : ''}`}>{index ? '✦' : '↗'}</span><span><b>{task.title}</b><small>#{task.id} · {task.assignee}</small></span>
            </button>
          ))}
        </div>
        <div className="side-section rules-box"><p>房间规则</p><div><span>↻</span><b>禁止无限转交</b><small>一轮后必须交付或询问用户</small></div></div>
        ...
        {activeView === 'chat' && (
          <>
            <div className="chat-scroll">
              ...
              <UserMessage text="帮我研究多 Agent 协作产品，给一个可以直接开工的方案。" mention="@所有老大" at="14:32" />
              ...
              {feedItems.map((item) => item.kind === 'message'
                ? <UserMessage key={item.id} text={item.text} at={item.at} />
...
function UserMessage({ text, mention, at }: { text: string; mention?: string; at: string }) {
  return <div className="message user-message"><div className="avatar user-avatar">用</div><div className="message-body"><div className="message-meta"><b>{USER_NAME}</b><span>用户</span><time>{at}</time></div><p>{text} {mention && <mark>{mention}</mark>}</p></div></div>;
}

function AgentMessage() {
  return <div className="message"><div className="avatar plum-avatar">M<span className="status-dot" /></div><div className="message-body"><div className="message-meta"><b>Mochi</b><span className="lead-tag">♛ 书房老大</span><time>14:33</time></div><p>我来负责。我会先画清任务协议，再请 <mark>@Nova</mark> 验证跨设备通信，15 分钟后给用户第一版。</p><div className="thinking"><i /><span>正在梳理产品架构</span><b>01:42</b></div></div></div>;
}

function TaskEvent({ task, onOpen, onGuard, guarded }: { task: Task; onOpen: () => void; onGuard?: () => void; guarded?: boolean }) {
  return <article className="task-event"><div className="event-top"><span className="event-icon">↗</span><div><small>任务 · #{task.id}</small><h2>{task.title}</h2></div><span className={`priority p-${task.priority}`}>{task.priority}优先级</span></div><div className="event-route"><span>用户发布</span><i>→</i><span className="chosen">{task.assignee} 接单</span><i>→</i><span>{task.progress}% 进行中</span></div><div className="event-foot"><span className="stacked-avatars"><i>M</i><i>N</i><i>L</i></span><span><b>{task.assignee}</b> {task.reason}</span><div className="event-buttons">{onGuard && <button onClick={onGuard}>{guarded ? '已触发保护' : '演示防踢皮球'}</button>}<button onClick={onOpen}>查看任务</button></div></div></article>;
}

function CheckpointCard({ outcome, onResolve }: { outcome: 'partial' | 'ask' | null; onResolve: (outcome: 'partial' | 'ask') => void }) {
  return <article className="checkpoint-card" aria-live="polite"><div className="checkpoint-head"><span>!</span><div><small>DECISION CHECKPOINT · 已阻止循环</small><h3>任务不能再被“踢皮球”</h3></div></div><div className="loop-route"><b>Mochi</b><i>→</i><b>Nova</b><i>→</i><b>Lumi</b><i>↛</i><b className="blocked-node">Mochi</b></div><p>系统发现下一位接收者已经在本轮路径中。当前负责人 Lumi 不能继续转交，必须提交现有结果，或把一个合并后的最小问题交给用户。</p>{!outcome ? <div className="checkpoint-actions"><button onClick={() => onResolve('partial')}>提交阶段结果</button><button className="dark" onClick={() => onResolve('ask')}>询问用户</button></div> : <div className="checkpoint-result"><span>✓</span><div><b>{outcome === 'partial' ? '已提交阶段结果' : '已进入等待用户'}</b><small>{outcome === 'partial' ? '包含已完成内容、信心和未解决风险。' : '只生成一次合并询问，并暂停本轮自动转交。'}</small></div></div>}</article>;
}
...
function TaskInspector({ task, loopGuarded, outcome, onBack, onGuard, onResolve }: { task: Task; loopGuarded: boolean; outcome: 'partial' | 'ask' | null; onBack: () => void; onGuard: () => void; onResolve: (outcome: 'partial' | 'ask') => void }) {
  return <div className="inspector task-inspector"><button className="back-button" onClick={onBack}>← 返回现场</button><span className="eyebrow">TASK INSPECTOR · #{task.id}</span><h2>{task.title}</h2><div className="task-status-row"><span className={`status-badge s-${task.status}`}>{task.status}</span><span>{task.progress}%</span><span>{task.eta}</span></div><div className="progress-track"><i style={{ width: `${task.progress}%` }} /></div><section className="inspector-section"><small>唯一负责人</small><div className="assignee-card"><span className="avatar plum-avatar">{task.assignee[0]}</span><div><b>{task.assignee}</b><small>{task.reason}</small></div></div></section><section className="inspector-section"><small>委派路径 · {task.path.length}/3</small><div className="task-path">{task.path.length ? task.path.map((node, index) => <span key={`${node}-${index}`}><b>{node}</b>{index < task.path.length - 1 && <i>→</i>}</span>) : <em>尚未分配</em>}</div><p className="policy-copy">同一 Agent 在一轮内只能成为负责人一次；命中已访问节点后，转交会被立即阻止。</p>{task.id === 'T-028' && !loopGuarded && <button className="primary-wide" onClick={onGuard}>模拟再次转交给 Mochi</button>}{task.id === 'T-028' && loopGuarded && <div className="guard-mini"><b>! 循环已阻止</b><small>Lumi ↛ Mochi · 只允许交付或询问用户</small>{!outcome && <div><button onClick={() => onResolve('partial')}>交付阶段结果</button><button onClick={() => onResolve('ask')}>询问用户</button></div>}{outcome && <p>✓ {outcome === 'partial' ? '阶段结果已提交' : '正在等待用户'}</p>}</div>}</section><section className="inspector-section"><small>审计摘要</small><ul className="activity-mini"><li><i />Mochi 自动接单 <time>14:33</time></li><li><i />Nova 接受协作咨询 <time>14:34</time></li><li><i />Lumi 完成规则复核 <time>14:37</time></li></ul></section></div>;
}

function TasksView({ tasks, onSelect, onGuard }: { tasks: Task[]; onSelect: (task: Task) => void; onGuard: () => void }) {
  return <div className="workspace-view tasks-view"><div className="view-header"><div><span className="eyebrow">TASK CONTROL</span><h1>任务不会凭空消失</h1><p>每个任务始终只有一个负责人；咨询和协作不会偷偷改变归属。</p></div><div className="header-stats"><span><b>{tasks.filter((task) => task.status === '执行中').length}</b>执行中</span><span><b>{tasks.filter((task) => task.status === '等待用户').length}</b>等待用户</span></div></div><div className="board">{statusOrder.map((status) => <section className="board-column" key={status}><div className="column-head"><span className={`column-dot d-${status}`} />{status}<b>{tasks.filter((task) => task.status === status).length}</b></div><div className="column-cards">{tasks.filter((task) => task.status === status).map((task) => <button className="board-card" key={task.id} onClick={() => onSelect(task)}><span className="board-id">#{task.id}<em className={`pri-${task.priority}`}>{task.priority}</em></span><h3>{task.title}</h3><div className="board-assignee"><span>{task.assignee[0]}</span><b>{task.assignee}</b><small>{task.eta}</small></div><div className="mini-progress"><i style={{ width: `${task.progress}%` }} /></div>{task.path.length > 1 && <div className="mini-path">{task.path.join(' → ')}</div>}</button>)}</div></section>)}</div><div className="rule-demo"><span>↻</span><div><b>闭环保护正在工作</b><p>如果 A → B → C 又想回到 A，系统会锁住转交并要求交付或询问用户。</p></div><button onClick={onGuard}>运行一次演示</button></div></div>;
}

function FleetView({ devices, routing, onRoute, onAgent }: { devices: Device[]; routing: boolean; onRoute: () => void; onAgent: (agent: Agent) => void }) {
  return <div className="workspace-view fleet-view"><div className="view-header"><div><span className="eyebrow">FLEET MAP</span><h1>任意数量设备，一个本机老大</h1><p>用户位于设备之上。每台电脑只需要注册一个 Gateway，跨设备消息由两端老大接力，但任务仍只有一个负责人。</p></div><button className="route-button" onClick={onRoute} disabled={routing}>{routing ? '跨设备路由中…' : '模拟跨设备派单'}</button></div><div className={`topology ${routing ? 'routing' : ''}`}><div className="user-node"><span>用</span><b>用户</b><small>全局规则与最终决定</small></div><div className="topology-lines"><i /><i /><i /><span /></div><div className="device-islands">{devices.map((device, deviceIndex) => <article className="device-island" key={device.id}><div className="island-head"><span>0{deviceIndex + 1}</span><div><b>{device.name}</b><small>{device.meta}</small></div><em>{device.load}% 负载</em></div><div className="island-agents">{device.agents.map((agent) => <button key={agent.id} className={agent.leader ? 'leader' : ''} onClick={() => onAgent(agent)}><span className={`mini-avatar ${agent.tone}`}>{agent.name[0]}</span><span><b>{agent.name}</b><small>{agent.truth}</small></span>{agent.leader && <em>♛ 老大</em>}</button>)}</div></article>)}</div>{routing && <div className="route-caption"><span className="route-pulse" />Mochi → Nova → Lumi：上下文已结构化接力</div>}</div><div className="fleet-rules"><div><span>1</span><b>每台只有一个老大</b><p>使用唯一身份与心跳租约，避免同一设备出现两个指挥者。</p></div><div><span>2</span><b>跨设备也保留负责人</b><p>通信可以接力，任务归属只有接收方明确接受后才改变。</p></div><div><span>3</span><b>离线不会静默丢单</b><p>设备失联时保留最后心跳，并等待恢复或请用户重新分配。</p></div></div></div>;
}
